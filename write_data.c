...
static void BOV_Header(int nfile, char *varname, char *datafile, Grid *grid);
  ...
  /* ------------------------------------------------------------------- */
    ...
    if (single_file){  /* -- single output file -- */
      ...
      for (nv = 0; nv < output->nvar; nv++) {
        ...
        WriteBinaryArray (Vpt, dsize, sz, fbin, output->stag_var[nv]);
        CloseBinaryFile (fbin, sz);
	BOV_Header(output->nfile, output->var_name[nv], filename, grid);
      }
    }
...
void BOV_Header(int nfile, char *varname, char *datafile, Grid *grid)
{
  int  nv, nvar;
  char filename[256];
  FILE *fp;
  ...
  sprintf (filename,"%s.%04d.bov",varname, nfile);
  fp = fopen (filename, "w");
  fprintf (fp,"TIME: %f\n",g_time);
