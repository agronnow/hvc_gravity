subroutine calc_center_of_mass(ilevel)
  use amr_commons
  use hydro_commons
  use cooling_module
  use mpi_mod
  ...
  real(dp)::ctime=0.0,cvel=0.0,cdist=0.0,ccom=0.0,cmass=0.0,ptime,pvel,pcom
...
!  write(*,*) "density: ", uold(ind_leaf(i),1), "trc density: ", uold(ind_leaf(i),imetal+1), " y: ", xx(i,2)!, " Local CoM: ", localcenterofmassy, " Local mass: ", localmass, " Global CoM: ", globalcenterofmassy, " Global mass: ", globalmass
  ...
  globalmass(ilevel-levelmin+1) = globalmass_level*dx_loc**ndim
  globalcenterofmassy(ilevel-levelmin+1) = globalcenterofmassy_level*dx_loc**ndim
#ifdef CALC_COMVEL
  globalcenterofmassvely(ilevel-levelmin+1) = globalcenterofmassvely_level*dx_loc**ndim
#endif
#else
  globalcenterofmassy(ilevel-levelmin+1) = localcenterofmassy_level*dx_loc**ndim
  globalmass(ilevel-levelmin+1) = localmass_level*dx_loc**ndim
#ifdef CALC_COMVEL
  globalcenterofmassvely(ilevel-levelmin+1) = localcenterofmassvely_level*dx_loc**ndim
#endif
#endif
  ...
  if (firstcall) then
                 ...
                 if(file_exist) then
                   ...
                   do
                     ...
                     pvel=cvel
                     pcom=ccom
                     read(150,*,end=100)ctime,cvel,cdist,ccom,cmass
                     if (ctime .ge. t) exit
                   end do
100 close(ilun)
                   t_prev = ptime
           ...
           !if ((vel_prev < 1.e-10) .or. (abs(cur_velocity/vel_prev) < 5.0)) then !Protect against spurious velocity changes
              ...
              if (myid==1) then
                 ...
                 if(.not.file_exist) then
                    ...
                    write(ilun, '(A53)') 'time frame_vel frame_dist frame_CoM M_c,total (M_sun)'
                 else
                    open(ilun, file=fileloc, status="old", position="append", action="write", form='formatted')
                 endif
                 write(ilun,'(5E26.16)') t, frame_velocity*scale_v/1.e5, frame_dist, frame_com, sum(globalmass)*scale_m/2e33
                 close(ilun)
              endif
  ...
  if (UpdateVel(ilevel-levelmin+1)) then
        ...
        UpdateVel(ilevel-levelmin+1) = .false.
   endif
