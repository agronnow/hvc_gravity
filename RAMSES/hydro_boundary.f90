...
!  real(dp),dimension(1:nvector),save::xb
  ...
  do ibound=1,nboundary
     ...
     if(boundary_type(ibound)==23)alt(1:8)=-(/2,2,1,1,2,2,1,1/)
     if(boundary_type(ibound)==24)alt(1:8)=+(/1,1,2,2,1,1,2,2/)
!     if(boundary_type(ibound)==23)alt(1:8)=+(/2,2,1,1,2,2,1,1/)
!     if(boundary_type(ibound)==24)alt(1:8)=-(/1,1,2,2,1,1,2,2/)
...
!!$              end if
           ...
           ! Imposed boundary conditions
              ...
              ! Changed from default: First set cell quantities equal to cells next to boundary
              do ivar=1,nvar
                 do i=1,ngrid
                    uu(i,ivar)=uold(ind_cell_ref(i),ivar)
                 end do
              ...
              !do i=1,ngrid
              !   xb(i)=xg(ind_cell_ref(i),2)+xc(ind,2)
              !end do

              if (gdim==2) then
                 !write(*,*) "Boundary dir: ", boundary_dir, " boundary level ", ilevel, " ngrid: ", ngrid, " dx: ", dx_loc, " alt: ", alt(ind)
                 call boundana(xx,-dx_loc*alt(ind),uu,dx_loc,ibound,ngrid)
              endif
