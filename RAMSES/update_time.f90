...
subroutine output_timer(write_file, filename)
  ...
  integer      :: ilun=11
  character(LEN=256)::filename, fileloc !Optional for writing timing info
!-----------------------------------------------------------------------
  id_is_one = myid == 1
  total = 1e-9
  ...
  if(nstep_coarse .ne. nstep_coarse_old)then
     ...
     if(t>=tout(noutput).or.aexp>=aout(noutput).or. &
        ...
        if(myid==1)then
           ...
           call totalruntime(ttend-ttstart)
#endif
        endif
        call clean_stop
     end if
...
!------------------------------------------------------------------------
subroutine totalruntime(deltaTime)
!
!
!   convert a floating-point variable (deltaTime, in seconds) to a string 
!   displaying days:hours:minutes:seconds
!
!   NOTE: This routine has been adapted from PLUTO v4.1 by TTG (Mar 2016)
!   
!------------------------------------------------------------------------
  integer::days,hours,mins,secs
  real(kind=8)::deltaTime

  days  = floor(deltatime/86400.0)
  hours = floor((deltatime - 86400.0*days)/3600.0)
  mins  = floor((deltatime - 86400.0*days - 3600.0*hours)/60.)
  secs  = ceiling(deltatime - 86400.0*days - 3600.0*hours - 60.0*mins)

  write(*,123) days,hours,mins,secs
  write(*,*)

  123 format(' Total elapsed time: ',i3,'d:',i3,'h:',i3,'m:',i3,'s')

end subroutine totalruntime
!------------------------------------------------------------------------
