subroutine backup_hydro(filename, filename_desc)
  ...
  character(len=256), intent(in) :: filename, filename_desc
  ...
  open(unit=unit_out, file=fileloc, form='unformatted')
  ...
  if (myid == 1) then
     ...
     open(unit=unit_info, file=filename_desc, form='formatted')
