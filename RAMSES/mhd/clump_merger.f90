...
subroutine write_clump_properties(to_file)
  ...
  character(LEN=256)::fileloc,filedir
...
subroutine write_clump_map
  ...
  if(IOGROUPSIZEREP>0)then
     call title(((myid-1)/IOGROUPSIZEREP)+1,ncharcpu)
     open(unit=20,file=trim(output_dir)//TRIM('output_'//TRIM(nchar)//'/group_'//TRIM(ncharcpu)//'/clump_map.csv'//myidstring),form='formatted')
  else
     open(unit=20,file=TRIM(trim(output_dir)//'output_'//TRIM(nchar)//'/clump_map.csv'//myidstring),form='formatted')
  endif
  !loop parts
  do ipart=1,ntest
