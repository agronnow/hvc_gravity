...
subroutine condinit(x,u,dx,nn)
...
#endif
     if(cooling_test)then
       ndens_wind = ndens_cloud/100.0
       ! Static uniform setup for testing the efficiency of cooling, combine with outflow boundary conditions everywhere
       ndens = ndens_wind + (ndens_cloud - ndens_wind)*0.5*(1.0-tanh(1.0+(currad-(rad_cloud+width))/width))
       nH = ndens*scale_nH
       temp = ndens_cloud*1.e4/ndens
...
#endif
       q(i,ndim+2) = ndens_cloud*1.e4/scale_T2
  ...
  ! radiative energy -> total fluid energy
  do irad=1,nener
     u(1:nn,ndim+2+irad)=q(1:nn,ndim+2+irad)/(gamma_rad(irad)-1.0d0)
